# 📄 Lumina IDE v5.2 — Project Y (Frontend-TS)

Este documento contém o código-fonte integral da refatoração Vanilla TypeScript (v5.2).
...
- [src\ui\ExtensionsPanel.ts](#srcuiExtensionsPanelts)
- [src\ui\LibraryPanel.ts](#srcuiLibraryPanelts)
- [src\ui\MeshPanel.ts](#srcuiMeshPanelts)
- [src\ui\Modals.ts](#srcuiModalsts)
- [src\ui\Resizer.ts](#srcuiResizerts)
...
// Use the Vite proxy: connect to the same host/port as the page
...
export class TerminalManager {
  ...
  private connect(): void {
    ...
    const wsUrl = `${WS_ORIGIN}/api/ws/terminal/default`;
...
import { fetchExtensions, toggleExtension } from '../api/client';
...
async function loadExtensions(): Promise<void> {
  ...
  try {
    ...
    if (!extensions || extensions.length === 0) {
      listEl.innerHTML = `
        <div style="text-align:center; padding:24px; color:var(--text-muted);">
          ...
          <div style="font-size:12px;">Nenhuma extensão instalada.</div>
          <div style="font-size:11px; margin-top:4px; color:var(--text-muted);">Coloque extensões na pasta <code>lumina_extensions/</code></div>
        </div>
      `;
      return;
    }
    ...
    listEl.innerHTML = '<p style="font-size:11px; color:var(--accent-red); padding:8px;">Erro ao carregar extensões.</p>';
  }
}
...
## src\ui\LibraryPanel.ts
```typescript
/**
 * Project Y — Library Panel (v5.2)
 * Document upload (drag & drop + file picker) and indexed document list.
 * Pure DOM manipulation — no frameworks.
 */

import { fetchLibraryList } from '../api/client';
...
export function initLibraryPanel(): void {
  ...
  panel.innerHTML = `
    ...
    <div style="padding:12px; display:flex; flex-direction:column; gap:12px; flex:1; overflow-y:auto;">
      <!-- Upload Zone -->
      <div id="projecty-upload-zone" class="upload-zone">
        <span class="upload-icon">📤</span>
        <span class="upload-text">Arraste arquivos aqui ou clique para selecionar</span>
        <span style="font-size:10px; color:var(--text-muted);">PDF, Markdown, TXT</span>
        <input id="projecty-upload-input" type="file" accept=".pdf,.md,.txt" multiple style="display:none;" />
      </div>
      <!-- Upload Status -->
      ...
      <!-- Document List -->
      <div>
        <h3 style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px; margin-bottom:8px;">Documentos Indexados</h3>
...
async function loadDocuments(): Promise<void> {
  ...
  try {
    ...
    if (docs.length === 0) {
      listEl.innerHTML = '<p style="font-size:11px; color:var(--text-muted); text-align:center;">Nenhum documento indexado.</p>';
      return;
    }

    listEl.innerHTML = '';
    for (const doc of docs) {
      const d = doc as { name?: string; filename?: string; size?: number; created_at?: string };
      const item = document.createElement('div');
      item.className = 'doc-item';
      const name = d.name || d.filename || 'Arquivo';
      const sizeKB = d.size ? `${(d.size / 1024).toFixed(1)} KB` : '';
      item.innerHTML = `
        <span class="doc-icon">📄</span>
        <div class="doc-info">
          <div class="doc-name">${name}</div>
          <div class="doc-meta">${sizeKB}</div>
        </div>
      `;
      listEl.appendChild(item);
    }
  } catch {
    listEl.innerHTML = '<p style="font-size:11px; color:var(--accent-red);">Erro ao carregar documentos.</p>';
  }
}
...
```typescript
/**
 * Project Y — Mesh Panel (v5.2)
 * Swarm network status and node discovery.
 * Pure DOM manipulation — no frameworks.
 */

import { fetchMeshStatus, fetchMeshNodes } from '../api/client';
import type { MeshNode } from '../api/client';
...
export function initMeshPanel(): void {
  ...
  panel.innerHTML = `
    ...
    <div style="padding:12px; display:flex; flex-direction:column; gap:12px; flex:1; overflow-y:auto;">
      ...
      <div style="background:var(--bg-overlay); border:1px solid var(--border); border-radius:6px; padding:12px;">
        <div style="font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:0.5px; color:var(--text-muted); margin-bottom:8px;">Status Local</div>
        <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:4px;">
          <span style="color:var(--text-secondary);">VRAM Livre</span>
      ...
      <div>
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
          <h3 style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px;">Nós Detectados</h3>
  ...
  refreshInterval = setInterval(refreshMesh, 5000);
}
...
async function refreshMesh(): Promise<void> {
  ...
  try {
    ...
    if (vramEl) vramEl.textContent = `${status.vram_free_gb.toFixed(1)} GB`;
    if (typeEl) typeEl.textContent = status.is_cortex ? '🧠 Nó Cortex' : '💡 Nó Leve';
  } catch { /* ignore */ }
  ...
  try {
    ...
    if (nodes.length === 0) {
      nodesEl.innerHTML = '<p style="font-size:11px; color:var(--text-muted); text-align:center; padding:8px;">Nenhum nó detectado na rede.</p>';
      return;
    }
...
function createNodeCard(node: MeshNode): HTMLElement {
  const card = document.createElement('div');
  card.style.cssText = `
    background: var(--bg-overlay); border: 1px solid var(--border);
  ...
  card.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <span style="font-size:12px; font-weight:600; color:var(--text);">${node.hostname || 'Desconhecido'}</span>
...
```typescript
/**
 * Project Y — Resizer (v5.2)
 * Drag-to-resize for sidebar width and terminal height.
...
function initSidebarResizer(): void {
  ...
  // Create the invisible drag handle at the right edge of the sidebar
...
/** Terminal horizontal resizer — changes terminal zone height */
function initTerminalResizer(): void {
  const resizer = document.getElementById('projecty-resize-h') as HTMLElement;
  const termZone = document.getElementById('projecty-terminal-zone') as HTMLElement;
  if (!resizer || !termZone) return;
  ...
  document.addEventListener('mousemove', (e: MouseEvent) => {
    ...
    termZone.style.height = `${newHeight}px`;
  });
