# Documentação de Código Fonte Frontend-TS (v5.0)

Esta documentação contém o código fonte completo da migração para Vanilla TypeScript da Lumina IDE v5.0.
...
```ts
/**
 * Project Y — Terminal Manager (v5.0)
 * Encapsulates xterm.js + WebSocket PTY tunnel.
 * The terminal DOM node is NEVER destroyed — it lives forever in #projecty-terminal-container.
 */
...
export class TerminalManager {
  ...
  /**
   * Mount the terminal into the DOM and connect to the PTY backend.
  ...
  private connect(): void {
    ...
    this.ws.onopen = () => {
      this.term.writeln('\x1B[1;32m--- Lumina Terminal Connected ---\x1B[0m');
