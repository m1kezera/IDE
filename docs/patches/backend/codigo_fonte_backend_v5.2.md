# 📄 Lumina IDE v5.2 — Project Y (Backend)

Este documento contém o código-fonte integral do motor nativo (v5.2).
...
- [agent.py](#agentpy)
- [brain.py](#brainpy)
- [chunker.py](#chunkerpy)
- [config.py](#configpy)
- [database.py](#databasepy)
- [edge_runtime.py](#edge_runtimepy)
- [extensions.py](#extensionspy)
- [healer.py](#healerpy)
- [identity.py](#identitypy)
- [knowledge_base.json](#knowledge_basejson)
- [library.py](#librarypy)
- [logger_utils.py](#logger_utilspy)
- [lumina-backend.spec](#lumina-backendspec)
- [main.py](#mainpy)
- [mesh.py](#meshpy)
- [models.py](#modelspy)
- [ollama_orchestrator.py](#ollama_orchestratorpy)
- [predictor.py](#predictorpy)
- [projecty-backend.spec](#projecty-backendspec)
- [pyrightconfig.json](#pyrightconfigjson)
- [requirements.txt](#requirementstxt)
- [router.py](#routerpy)
- [security_vault.py](#security_vaultpy)
- [sentinel.py](#sentinelpy)
- [setup_manager.py](#setup_managerpy)
- [telemetry.py](#telemetrypy)
- [terminal.py](#terminalpy)
- [watcher.py](#watcherpy)
...
## .env
```text
PROJECTY_ENC:gAAAAABpswzQ71k9HDc3IayiJaD2SJK-vlVy_xwBQlLPpUDjs65ymUyPljDQ0060yjrDkduhUX7Hzafk-y_6uZh9ibYQ9fjSDn5Vc0_FVZxMU7ILbYUtjn87U85z-XRObS2TqtH6pNVKZIkb9cKrBISy4_P6OwRBDg6rs9B8DqkaX01uvb5a86GAMfK_FvWmGI4Ct_UoSMMbT4_dkSYZdX6qwJaMbFYbNq0eOh0ay6jXTai0VmliT1xggUvyMU6SZ9u5JGKYNjUJjNlzwLQZsHPaQs05HiTFRdRd__TK9hWCLY5QhAz4vdJuCbnOmpK4dXKdrro80VstP5xEb9rOOdGjeksBwE_YzqxItxfrWrfNSCyRt5Ff8uDPjKWN8uKRU3j6h182UefDj6qUfaGRZjtvN7zFRcanDr-QcPM7PgiRbxUXWkwUXfcR6k5TW-mYEsogOYVxZLg5
```

## agent.py
```python
"""Project Y — Agentic File Operations.

Parses AI model responses for file blocks and writes them to the workspace.
This is what makes Project Y act like a coding agent instead of a chatbot.
...
SYSTEM_PROMPT = """You are Project Y, an expert AI coding agent embedded in the Project Y. You CREATE, MODIFY, and DELETE files directly in the user's workspace.
...
[ ] Write a task...
[ ] Another task...
(The system will write this to PROJECTY_PLAN.md automatically)
...
def parse_file_blocks(text: str) -> list[dict]:
    ...
    for match in _PLAN_RE.finditer(text):
        ...
        if plan_content:
            blocks.append({"path": "PROJECTY_PLAN.md", "content": plan_content, "op": "plan"})
...
def write_file_blocks(workspace_path: str, blocks: list[dict]) -> list[dict]:
    ...
    for block in blocks:
        ...
        if block.get("op") == "plan":
            try:
                plan_path = os.path.join(workspace_path, "PROJECTY_PLAN.md")
                existed = os.path.isfile(plan_path)
                with open(plan_path, "w", encoding="utf-8", newline="\n") as f:
                    f.write(block["content"])
                results.append({"path": "PROJECTY_PLAN.md", "status": "modified" if existed else "created", "size": len(block["content"])})
            except Exception as exc:
                results.append({"path": "PROJECTY_PLAN.md", "status": "error", "error": str(exc)})
            continue
    ...
    # Check off modified files in PROJECTY_PLAN.md
    try:
        plan_path = os.path.join(workspace_path, "PROJECTY_PLAN.md")
        if os.path.isfile(plan_path):
            ...
            for res in results:
                if res["status"] in ("created", "modified", "deleted") and res["path"] != "PROJECTY_PLAN.md":
...
<html lang="en">
  <head>
    ...
    <title>Project Y Generated App</title>
  </head>
  <body>
    <div id="root"></div>
...
class ProjectYBrain:
    def __init__(self):
        ...
        if not _DEPS_OK:
            log.warning("🧠 Brain em modo degradado (dependências faltando)")
            return
...
def _resolve_extensions_dir() -> str:
    ...
    Priority:
      ...
      2. %APPDATA%/projecty-ide/extensions  (Windows standard)
    ...
    if env_override:
        ...
        appdata = os.environ.get("APPDATA", os.path.expanduser("~"))
        target = os.path.join(appdata, "projecty-ide", "extensions")
    else:
        # macOS / Linux fallback
        target = os.path.join(os.path.expanduser("~"), ".projecty-ide", "extensions")
...
async def lifespan(app: FastAPI):
    ...
    log.info("═══════════════════════════════════════════════")
    log.info("   🚀 Project Y Backend — Iniciando...")
    log.info("═══════════════════════════════════════════════")
    ...
    log.info("═══════════════════════════════════════════════")
    log.info("   ✅ Backend pronto! http://127.0.0.1:8001")
    log.info("═══════════════════════════════════════════════")

    # Start Project Y Swarm Mesh (Module A)
    try:
        import mesh
        mesh.start_mesh(port=8001)
    except Exception as e:
        log.error(f"❌ Falha ao iniciar Project Y Mesh: {e}")

    yield

    log.info("🔴 Project Y Backend — Encerrando...")
    try:
...
def _start_mesh_internal(port: int):
    ...
    try:
        ...
        logger.info(f"🚀 Project Y Mesh (v5.0) Broadcast Active on {local_ip}:{port}")
    except Exception as e:
        import traceback
...
async def generate_prediction(req: PredictionRequest) -> Dict[str, Any]:
    ...
    if load_tier == "critical":
        ...
        if cortex:
            import logging
            logging.getLogger("projecty.predictor").info(f"🕸️ Swarm Delegation: Proxying prediction to {cortex.ip}:{cortex.port}")
    ...
    if free_ram_gb < 4.0:
        ...
        if cortex:
            import logging
            logging.getLogger("projecty.predictor").info(f"🕸️ Swarm Delegation (< 4GB VRAM): Proxying prediction to {cortex.ip}:{cortex.port}")
            return await mesh.proxy_prediction(cortex, req.model_dump())
...
async def health():
    """Simple health check."""
    log.info("💓 /health — OK")
    return {"status": "ok", "service": "projecty-backend"}
...
async def get_brain_status():
    """Reports the status of the Project Y Brain engine and Pulse metrics."""
    from watcher import get_watcher
    watcher = get_watcher()
...
def run_command_in_terminal(command: str, port: str = "cmd") -> bool:
    ...
    if not command.endswith('\n'):
        # In a PTY environment, Windows typically expects \r or \r\n to execute commands
        command += '\r\n'
...
async def terminal_websocket(websocket: WebSocket, port: str):
    await websocket.accept()
    loop = asyncio.get_running_loop()

    try:
        # Cria o Pseudo-Terminal (PTY) apontando nativamente para o PowerShell do Windows 10
        pty_process = winpty.PTY(80, 24)
        pty_process.spawn("powershell.exe")
    ...
    async def read_from_pty():
        ...
        while True:
            try:
                # O pty_process.read já captura as cores (ANSI) nativas do Windows
                data = await loop.run_in_executor(None, pty_process.read)
                if not data:
                    break
                    
                # Store history
                TERMINAL_HISTORY[port] += data
                if len(TERMINAL_HISTORY[port]) > 50000:
                    TERMINAL_HISTORY[port] = TERMINAL_HISTORY[port][-50000:]
    ...
    try:
        while True:
            ...
            is_command = False
            # Verifica se o texto recebido é um JSON de controle (ex: resize)
            if client_data.startswith("{") and "type" in client_data:
                try:
                    req = json.loads(client_data)
                    if req.get("type") == "resize":
                        cols = req.get("cols", 80)
                        rows = req.get("rows", 24)
                        pty_process.set_size(cols, rows) # Altera a janela do Windows nativamente
                        is_command = True
                except json.JSONDecodeError:
                    pass # Se não for JSON válido, segue o jogo
            
            # Se NÃO for um comando de controle, injeta como tecla digitada no PowerShell
            if not is_command:
                await loop.run_in_executor(None, pty_process.write, client_data)
                
    except WebSocketDisconnect:
        pass  # Você fechou a aba ou recarregou a página
    finally:
        # Limpeza para não deixar o powershell.exe rodando como "zumbi" na RAM
        reader_task.cancel()
