# Documentação de Código Fonte Frontend (v2.6.1)
...
function TelemetrySidebarContent({ brainData }) {
...
export default function BottomPanel({ mode, onFilesCreated, promptFiles = [] }) {
    ...
    const initWebSocket = (termId) => {
        ...
        const wsUrl = `ws://${WS_HOST}/api/ws/terminal/${portParam}`;
...
### `src\components\HealerOverlay.jsx`
```javascript
...
### `src\components\IdentityModal.jsx`
```javascript
...
### `src\components\SecurityAlert.jsx`
```javascript
...
// ─── SSE Streaming ──────────────────────────────────────────────────
export function streamGenerate({ prompt, mode, model, format }, { onToken, onDone, onError, onFiles, onPendingConfirmation }) {
    const controller = new AbortController();
