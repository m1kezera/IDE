# Project Y v2.4.5 — Frontend Source Documentation
...
## 📄 vite.config.ts
**Path**: `c:\pulsyce\frontend\vite.config.ts`
...
## 📄 index.html
**Path**: `c:\pulsyce\frontend\index.html`
...
## 📄 main.js
**Path**: `c:\pulsyce\electron\main.js`
...
## 📄 preload.js
**Path**: `c:\pulsyce\electron\preload.js`

```javascript
const { contextBridge } = require('electron');

// We expose nothing dangerous to the renderer process
// Standard contextIsolation setup
contextBridge.exposeInMainWorld('electronAPI', {
    // Add required bridge methods here if needed
});
...
**Path**: `c:\pulsyce\electron-builder.yml`
...
export default function App() {
    ...
    useEffect(() => {
        ...
        // Check Project Y Shield Identity (v2.2.2: Isolated from polling)
...
export const checkIdentityStatus = async (retries = 30, delay = 2000) => {
    for (let i = 0; i < retries; i++) {
        try {
            const res = await fetch(`${BASE}/identity/check`);
            if (res.ok) {
                const data = await res.json();
                return data; // Valid response — return immediately (registered or not)
            }
            // 404 means router not loaded yet, keep retrying
