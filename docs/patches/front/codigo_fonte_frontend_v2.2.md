# Código Fonte Frontend Completo - Lumina IDE (v2.2)

> Este documento representa a versão 2.2 (Lumina Speed & Pulse), consolidando a visualização de latência do Nervo Óptico e animações de Pulso do Brain.
...
export default function StatusBar({ mode, model, workspace, tokenCount, bypassProtection, memoryStatus, onEdgeClick, userName, sentinelStatus = 'active', brainData }) {
