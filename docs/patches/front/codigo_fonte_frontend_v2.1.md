# Código Fonte Frontend Completo - Lumina IDE (v2.1)

> Este documento representa a versão 2.1 (Lumina Sentinel), consolidando o Dashboard de Segurança e o sistema de Alertas Críticos (Panic Mode).

## Frontend - Interface React da Lumina IDE v2.1 (Sentinel UI & Security Alerts)
...
export default function App() {
    ...
    useEffect(() => {
        ...
        // Check Lumina Shield Identity
        checkIdentityStatus().then(setIdentity);

        // Sentinel Sentinel Monitor (v2.1)
        ...
        return () => {
            clearInterval(configInterval); // configInterval might not be set if fetchConfig is not polling
            clearInterval(sentinelInterval);
        };
    }, []); // Empty dependency array for initial load and polling setup
    ...
    return (
        <div className="ide-layout">
            ...
            <StatusBar
                ...
                // The following props were in the original code but not in the diff's StatusBar example.
                // Keeping them as they were in the original code.
                // bypassProtection={bypassProtection} // bypassProtection is not defined in the original code either
                // memoryStatus={memoryStatus} // memoryStatus is not defined in the original code either
            ...
            <OnboardingModal onComplete={() => setMode('local')} />
        </div>
    );
}
            ...
            <div className="live-preview-content" style={{ flex: 1, position: 'relative', overflow: 'hidden', padding: '10px' }}>
                ...
                {status === 'starting' && (
                    <div style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <span style={{ marginLeft: '12px' }}>Preparando Sandbox...</span>
                    </div>
                )}
...
export default function StatusBar({ mode, model, workspace, tokenCount, bypassProtection, memoryStatus, onEdgeClick, userName, sentinelStatus = 'active' }) {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            fetchBrainStatus().then(data => setBrainStatus(data.status || 'OFFLINE'));
        }, 5000);
        return () => clearInterval(interval);
    }, []);
        ...
        <div className="status-bar">
            ...
            <div className="status-right">
                <span className="status-item" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    {mode === 'local' ? <Zap size={12} /> : <Box size={12} />} {mode === 'local' ? 'Ollama' : 'Cloud'}
                </span>
                {model && (
                ...
                {/* Brain Status (v6.0) */}
                <span className="status-item" title={brainStatus === 'READY' ? "Memória Persistente Ativa" : "Indexando Código..."} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Brain size={12} color={brainStatus === 'READY' ? 'var(--lumina-blue)' : 'var(--accent-orange)'} />
                    BRAIN: <span style={{ color: brainStatus === 'READY' ? 'var(--lumina-blue)' : 'var(--accent-orange)' }}>{brainStatus}</span>
                </span>
                {/* Library Status (v6.5) */}
...
// ─── Lumina Brain (v6.0) ───────────────────────────────────────────
export const fetchBrainStatus = () => fetch(`${BASE}/brain/status`).then(r => r.json()).catch(() => ({ status: 'OFFLINE', is_ready: false }));
...
## Electron - Configurações de Desktop da Lumina IDE v2.1
...
function getBackendCommand() {
    if (IS_PACKAGED) {
        ...
        return { command: exePath, args: [], cwd: path.join(process.resourcesPath, 'backend', 'lumina-backend') };
    }
...
app.whenReady().then(async () => {
    ...
    if (!ready) {
        console.warn('[Electron] Starting UI anyway (backend may still be loading)...');
    }
