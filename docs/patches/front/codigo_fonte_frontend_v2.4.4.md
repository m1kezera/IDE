# Código Fonte Frontend Completo - Project Y (v2.4.4)

> Este documento contém o dump de todo o código-fonte atual exclusivo do Frontend (React + Vite) e do Electron do Project Y, refletindo a versão 2.4.4 com diagnóstico blackbox, handshake resiliente e rebrand completo.

## Frontend - Interface React do Project Y v2.4.4 (Diagnostic & Rebrand)
...
export default function IdentityModal({ onRegister }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(async () => {
            try {
                const resp = await fetch(`${BASE_HOST}/api/identity/register`, {
                    method: 'POST',
                ...
                console.error(err);
                alert("Falha na conexão com o Project Y Shield.");
                setStep(1);
            } finally {
                setLoading(false);
            }
...
## Electron - Configurações de Desktop do Project Y v2.4.4
