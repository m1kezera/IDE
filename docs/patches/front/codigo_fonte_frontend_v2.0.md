# Código Fonte Frontend Completo - Lumina IDE (v2.0)

> Este documento representa a versão 2.0 (Lumina Shield), consolidando a interface visual premium e o sistema de onboarding de identidade.

## Frontend - Interface React da Lumina IDE v2.0 (Lumina Shield & Visual Overhaul)
...
import { fetchConfig, fetchDashboard, getFileTree, readFile, predictNextSteps } from './services/api';
...
export default function App() {
    ...
    const [identity, setIdentity] = useState({ is_registered: true, user_name: null }); // Default to true to prevent flash
    ...
    useEffect(() => {
        ...
        fetch('/api/identity/check')
            ...
            .catch(err => console.error("Falha ao verificar identidade:", err));
    }, []);
...
export default function StatusBar({ mode, model, workspace, tokenCount, bypassProtection, memoryStatus, onEdgeClick, userName }) {
...
## Electron - Configurações de Desktop da Lumina IDE v2.0
