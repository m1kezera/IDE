# Código Fonte Frontend Completo - Lumina IDE (v2.3.2)

> Este documento contém o dump de todo o código-fonte atual exclusivo do Frontend (React + Vite) e do Electron da Lumina IDE, refletindo a versão 2.3.2 com UX refinado e handshake estabilizado.

## Frontend - Interface React da Lumina IDE v2.3.2 (SeatCode Release)
...
export default function ModelManager() {
    ...
    const getBadgeInfo = (name, size) => {
        ...
        if (params >= 7 || size.includes('GB') && parseFloat(size) > 4) return { label: 'PRO', color: 'var(--lumina-blue)', text: 'var(--bg-base)' };
            ...
            <div className="status-right">
                ...
                >
                    <Zap
                        ...
                        color={motorStatus === 'ready' ? 'var(--lumina-blue)' : motorStatus === 'starting' ? '#ffb088' : 'var(--text-muted)'}
                    />
...
::-webkit-scrollbar-thumb:hover {
  background: var(--lumina-blue) !important;
...
@keyframes motorReadyFlash {
  ...
  30% { transform: scale(1.3); filter: brightness(2) drop-shadow(0 0 20px var(--lumina-blue)); }
  100% { transform: scale(1); filter: brightness(1) drop-shadow(0 0 5px var(--lumina-blue)); }
}
...
## Electron - Configurações de Desktop da Lumina IDE v2.3.2
...
function getBackendCommand() {
    if (IS_PACKAGED) {
        // In production, the backend exe lives inside resources/backend/lumina-backend
        const exePath = path.join(process.resourcesPath, 'backend', 'lumina-backend', 'lumina-backend.exe');
...
function killBackend() {
    ...
    try {
        ...
        if (process.platform === 'win32') {
            ...
            spawn('taskkill', ['/IM', 'lumina-backend.exe', '/F', '/T'], { stdio: 'ignore', windowsHide: true });
        } else {
