# Código Fonte Frontend Completo - Lumina IDE (v1.12)

> Este documento contém o dump de todo o código-fonte atual do Frontend da Lumina IDE, refletindo a versão 1.12 com a implementação do Lumina Edge (Live Preview, Sandbox Wasm UI e Deploy QR Code).

## Frontend - Interface da Lumina IDE v1.12 (Edge Runtime UI)
...
export default function FileEditor({ file, activePrediction, onClose, onSaved, activeHealerError }) {
    ...
    const handleKeyDown = (e) => {
        ...
        if (e.key === 'Tab') {
            ...
            if (activePrediction && activePrediction.ghost_block && content.trim() === '') {
                e.preventDefault();
                setContent(activePrediction.ghost_block);
                setModified(true);
                return;
            }
            e.preventDefault();
        ...
        <div className="file-editor">
            ...
            <div className="code-area">
                ...
                <div className="code-editor-wrapper">
                    ...
                    {/* Predictive Cache Ghost Block Overlay (Module C) */}
                    {activePrediction && activePrediction.ghost_block && content.trim() === '' && (
                        <div className="ghost-block-overlay">
                            <pre className="ghost-block-text">{activePrediction.ghost_block}</pre>
                            <div className="ghost-block-hint">✨ Predição de Contexto Pronta (Pressione <b>Tab ↹</b> para materializar)</div>
                        </div>
                    )}
                </div>
            </div>
        ...
        <div className="status-bar">
            ...
            <div className="status-right">
                ...
                <span className="status-item" title={isCortex ? "Cortex Node (Alto Processamento)" : "Terminal Node (Processamento Delegado)"}>
                    🕸️ Mesh: {meshNodes} {meshNodes === 1 ? 'nó' : 'nós'} {isCortex ? '(Cortex)' : '(Terminal)'}
                </span>
                {/* Edge QR Shortcut */}
