# Código Fonte Frontend Completo - Lumina IDE (v1.14)

> Este documento contém o dump de todo o código-fonte atual do Frontend da Lumina IDE, refletindo a versão 1.14 com a implementação do painel de Biblioteca (Lumina Library UI).

## Frontend - Interface da Lumina IDE v1.14 (Library & Documentation UI)
...
const ITEMS = [
    ...
    { id: 'swarm', icon: '🕸️', label: 'Lumina Mesh', shortcut: '' },
    { id: 'edge', icon: '⚡', label: 'Edge Preview', shortcut: 'Ctrl+Shift+L' },
    { id: 'library', icon: '📚', label: 'Library', shortcut: 'Ctrl+Shift+B' },
    ...
    return (
        <div className="library-panel panel-content">
            ...
            <div className="library-list">
                <h3>Documentos Aprendidos ({docs.length})</h3>
                {docs.length === 0 ? (
    ...
    return (
        <div className="status-bar">
            ...
            <div className="status-right">
                ...
                {/* Brain Status (v6.0) */}
                <span className="status-item" title={brainStatus === 'READY' ? "Memória Persistente Ativa" : "Indexando Código..."}>
                    🧠 BRAIN: <span style={{ color: brainStatus === 'READY' ? 'var(--accent-blue)' : 'var(--accent-orange)' }}>{brainStatus}</span>
                </span>
                {/* Library Status (v6.5) */}
                <span className="status-item" title="Biblioteca de Documentação Oficial">
                    📚 LIBRARY: <span style={{ color: 'var(--accent-green)' }}>ACTIVE</span>
                </span>
                {/* Mesh Indicator */}
                <span className="status-item" title={isCortex ? "Cortex Node (Alto Processamento)" : "Terminal Node (Processamento Delegado)"}>
                    🕸️ Mesh: {meshNodes} {isCortex ? '(Cortex)' : '(Terminal)'}
                </span>
                {/* Edge QR Shortcut */}
                <span className="status-item clickable" onClick={onEdgeClick} title="Lumina Edge: Acesso Remoto via QR Code">
                    📱 QR Deploy
                </span>
                {memoryStatus && (
