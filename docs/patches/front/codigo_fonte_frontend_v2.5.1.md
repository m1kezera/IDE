# Código Fonte Frontend — Project Y (v2.5.1)
    ...
    return (
        <>
            <div style={{ background: 'var(--bg-overlay)', borderRadius: '6px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
                    <span style={{ color: 'var(--text-muted)' }}>Custo Local (Project Y)</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginTop: '4px', borderTop: '1px solid var(--border)', paddingTop: '4px' }}>
                    ...
                    <span style={{ color: 'var(--projecty-blue)', fontWeight: 600 }}>{brainData?.analysis_latency_ms?.toFixed(2) || 0} ms</span>
                </div>
            </div>
...
export default function App() {
    ...
    useEffect(() => {
        ...
        // Phase 1.2: Strict Boot Flow (Handshake Seguro)
        const bootSequence = async () => {
            while (true) {
                try {
                    const health = await checkHealth();
                    if (health && health.status === 'ok') {
                        // Backend is safely up. Now fetch identity.
                        try {
                            const idData = await checkIdentityStatus();
                            setIdentity(idData);
                        } catch (e) {
                            // Backend up, but identity missing or failed
                            setIdentity({ is_registered: false, user_name: '' });
                        }
                        setAppStatus('ready');
                        break; // Exit boot loop
                    }
                } catch (e) {
                    // Backend offline or timeout -> keep looping
                }
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ws = new WebSocket('ws://127.0.0.1:8001/api/ws/watcher');
    ...
    const handleOpenSettings = useCallback(() => {
        const settingsPath = 'projecty://settings';
    ...
    if (appStatus === 'booting') {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw', background: 'var(--bg-base)', alignItems: 'center', justifyContent: 'center', color: 'var(--text)' }}>
                <div className="loading-spinner" style={{ width: '40px', height: '40px', marginBottom: '20px' }}></div>
                <h2 style={{ color: 'var(--text)', fontWeight: 600, letterSpacing: '2px' }}>AQUECENDO MOTORES DA IA</h2>
                <p style={{ color: 'var(--text-muted)', fontSize: '12px', marginTop: '8px' }}>Estabelecendo handshake seguro com a API local...</p>
            </div>
        );
    }
    ...
    return (
        <div className="ide-layout">
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden', gridArea: 'workspace' }}>
                <Group direction="horizontal" autoSaveId="projecty-layout-v1-h">
                    {activePanel ? (
                            <Panel
                                ref={sidePanelRef}
                                className={`side-panel glass-panel ${!brainData?.is_ready ? 'border-breathe' : ''}`}
                                defaultSize={20} minSize={15} maxSize={40} id="sidebar-panel">
                    ...
                    {activePanel && <Separator className="resizer-h" style={{ width: '4px', cursor: 'col-resize', background: 'var(--border)' }} />}

                    <Panel id="main-editor-panel">
                        <Group direction="vertical" autoSaveId="projecty-layout-v1-v">
                            <Panel id="editor-content-panel">
                                <main ref={editorAreaRef} className="editor-area" style={{ width: '100%', height: '100%' }}>
                                    <TabBar
                                        tabs={openTabs}
                                        activeTab={activeTab}
                                    ...
                                    <div className="editor-content">
                                        {activeTab === 'projecty://settings' ? (
                                            ...
                                            <div className="welcome-screen">
                                                ...
                                                <h1 className="welcome-title neon-blue">Project Y</h1>
                            ...
                            {bottomVisible && <Separator className="resizer-v" style={{ height: '4px', cursor: 'row-resize', background: 'var(--border)' }} />}
                            {bottomVisible && (
...
const savedTheme = localStorage.getItem('projecty-theme');
if (savedTheme === 'light') {
...
@keyframes motorReadyFlash {
  ...
  30% { transform: scale(1.3); filter: brightness(2) drop-shadow(0 0 20px var(--projecty-blue)); }
  100% { transform: scale(1); filter: brightness(1) drop-shadow(0 0 5px var(--projecty-blue)); }
}
...
@keyframes borderBreatheAnim {
  0% { opacity: 0.1; box-shadow: inset 0 0 5px rgba(0, 163, 255, 0.1); }
  50% { opacity: 0.4; box-shadow: inset 0 0 15px rgba(0, 163, 255, 0.3); }
  100% { opacity: 0.1; box-shadow: inset 0 0 5px rgba(0, 163, 255, 0.1); }
}
...
const ITEMS = [
    ...
    { id: 'swarm', icon: Share2, label: 'Project Y Mesh', shortcut: '' },
...
const WS_HOST = isElectron ? '127.0.0.1:8001' : '127.0.0.1:8001';
const HTTP_HOST = isElectron ? 'http://127.0.0.1:8001' : '';
...
export default function BottomPanel({ mode, onFilesCreated, promptFiles = [] }) {
    ...
    useEffect(() => {
        const url = `${HTTP_HOST}/api/system/logs`;
    ...
    useEffect(() => {
        import('../services/api').then(api => {
            api.fetchModels?.().then(data => {
                const m = data?.models?.find(x => x.name === window.localStorage.getItem('projecty_model') || x.name.includes('llama3') || x.name === 'mistral');
    ...
    const initWebSocket = (termId) => {
        ...
        const wsUrl = `ws://${WS_HOST}/api/ws/${portParam}`;
        ...
        ws.onopen = () => term.writeln('--- Project Y Terminal Connected ---');
    ...
    const handleSilentCorrection = (msg) => {
        ...
        const header = `${sep}▶ [Auto-Correction Triggered]\nProject Y detected a terminal error and is fixing it...\n\n◀ Project Y:\n`;
                    ...
                    <div ref={outputRef} onScroll={handleOutputScroll} className={`bottom-output ${streaming ? 'streaming' : ''}`} style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                        {output ? (
                            ...
                            <div className="bottom-welcome">
                                <span>🛰️ Project Y Agent — Digite um prompt abaixo. Abra uma pasta no Explorer para criar arquivos.</span>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <>
                        <div className="bottom-prompt-input-row" style={{ position: 'relative' }}>
                            ...
                            <textarea
                                ...
                                placeholder="Peça ao Project Y... (Enter envia, Shift+Enter nova linha)"
                                value={prompt}
...
export default class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: '20px', background: 'var(--bg-overlay)', margin: '10px', borderRadius: '8px', border: '1px solid var(--accent-red)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <h3 style={{ color: 'var(--accent-red)', margin: 0 }}>⚠️ Erro no Componente</h3>
          <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: 0 }}>Ocorreu uma falha isolada. A interface principal foi protegida da "tela branca".</p>
          <pre style={{ fontSize: '10px', color: 'var(--text)', background: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '4px', overflowX: 'auto', margin: 0 }}>
            {this.state.error?.message}
          </pre>
          <button 
            onClick={() => this.setState({ hasError: false, error: null })}
            style={{ padding: '6px 12px', background: 'var(--accent)', border: 'none', color: '#fff', borderRadius: '4px', cursor: 'pointer', alignSelf: 'flex-start', fontSize: '11px', fontWeight: 600 }}
          >
        ...
        <div className="panel-placeholder">
            ...
            <div className="extensions-menu">
                <p className="panel-hint" style={{ padding: '0 12px 12px' }}>
                    🧩 Adicione funcionalidades à Project Y via extensões locais.
                </p>
        ...
        <div className="file-editor">
            <div className="editor-toolbar">
                ...
                <div className="editor-toolbar-right">
                    ...
                    {usedBrain && (
                        <span className="brain-memory-indicator" title="Project Y está usando Memória Persistente (Brain v6.0) para esta sugestão">
                            💡
                        </span>
                    )}
...
export default function IdentityModal({ onRegister }) {
    ...
    const handleSubmit = async (e) => {
        ...
        // Wait for the scanning animation, then attempt registration with retries
        setTimeout(async () => {
            const maxRetries = 5;
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    const result = await registerIdentity(name);
                    if (result.success) {
                        onRegister(name);
                        return;
                    } else {
                        alert("Erro ao registrar identidade: " + (result.detail || 'Erro desconhecido'));
                        setStep(1);
                        setLoading(false);
                        return;
                    }
                } catch (err) {
                    console.warn(`[Shield] Tentativa ${attempt + 1}/${maxRetries} falhou:`, err.message);
                    if (attempt < maxRetries - 1) {
                        await new Promise(r => setTimeout(r, 3000));
                    }
                }
            }
            alert("Falha na conexão com o Project Y Shield. Tente novamente.");
            setStep(1);
            setLoading(false);
        }, 2500);
    };
...
export default function LibraryPanel() {
    ...
    const handleUpload = async (file) => {
        ...
        try {
            const resp = await fetch(`${BASE_HOST}/api/library/upload`, {
                method: 'POST',
                body: formData
            });
        ...
        <div className="library-panel panel-content">
            <div className="panel-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Library size={18} color="var(--projecty-blue)" />
            ...
            >
                {uploading ? (
                    <div className="upload-loader" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                        <Loader2 size={32} className="spinning" color="var(--projecty-blue)" />
                        <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Indexando no Brain...</span>
                    </div>
                ) : (
                    <>
                        <UploadCloud size={32} color={dragActive ? 'var(--projecty-blue)' : 'var(--text-muted)'} />
            ...
            <div className="library-list">
                <h3 style={{ color: 'var(--projecty-blue)', opacity: 0.8 }}>Docs Aprendidos ({docs.length})</h3>
                {docs.length === 0 ? (
            ...
            <div className="live-preview-content" style={{ flex: 1, position: 'relative', overflow: 'hidden', padding: '10px' }}>
                ...
                {status === 'starting' && (
                    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <span style={{ marginTop: '16px', color: 'var(--projecty-blue)', fontWeight: 600 }}>AGUARDANDO RUNTIME...</span>
                ...
                {status === 'running' && (
                    <div className="preview-frame-wrapper" style={{ height: '100%', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden', background: '#fff', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ height: '24px', background: '#f8f9fa', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 10px', color: '#000', fontSize: '10px', borderBottom: '1px solid #ddd' }}>
                            <span>Project Y Sim {signal}G</span>
                            <span>12:00</span>
                            <span>🔋 {battery}%</span>
                        </div>
                        <iframe
                            ...
                            title="Project Y Edge Live Preview"
                        />
                    </div>
                )}
...
### `components\ModelManager.jsx`
```javascript
import React, { useState, useEffect } from 'react';
import { fetchModelCatalog, streamPullModel, deleteModel } from '../services/api';
...
export default function ModelManager() {
    ...
    const getBadgeInfo = (name, size) => {
        const lowerName = name.toLowerCase();
        // Extract parameter size if possible (e.g., mistral:7b)
        const paramMatch = lowerName.match(/(\d+)b/);
        const params = paramMatch ? parseInt(paramMatch[1]) : 0;

        if (params >= 30) return { label: 'ELITE', color: 'linear-gradient(135deg, #b088ff, #70c4ff)', text: 'var(--bg-base)' };
        if (params >= 7 || size.includes('GB') && parseFloat(size) > 4) return { label: 'PRO', color: 'var(--projecty-blue)', text: 'var(--bg-base)' };
        return { label: 'ECO', color: 'var(--accent-green)', text: 'var(--bg-base)' };
    };
    ...
    return (
        <div className="model-manager" style={{ padding: '20px', color: 'var(--text)' }}>
            <h2 style={{ fontSize: '18px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Download size={20} color="var(--projecty-blue)" /> Project Y Orchestrator: Model Manager
            </h2>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '15px' }}>
                {catalog.map(model => {
                    const isInstalled = installed.some(i => i.startsWith(model.name));
                    const badge = getBadgeInfo(model.name, model.size);

                    return (
                        <div key={model.name} style={{
                            background: 'var(--bg-overlay)',
                            border: `1px solid ${isInstalled ? badge.color : 'var(--border)'}`,
                            borderRadius: '8px',
                            padding: '16px',
                            display: 'flex',
                            ...
                            <div style={{ marginTop: '5px', display: 'flex', gap: '8px' }}>
                                {isInstalled ? (
                                    ...
                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: 'var(--projecty-blue)' }}>
                                        ...
                                        <div style={{ height: '4px', background: 'var(--bg-base)', borderRadius: '2px', overflow: 'hidden' }}>
                                            <div style={{ height: '100%', background: 'var(--projecty-blue)', width: '40%', transition: 'width 0.3s' }}></div>
                                        </div>
                                    </div>
                                ) : (
            ...
            <div style={{ marginTop: '30px', padding: '15px', background: 'rgba(0,163,255,0.05)', borderRadius: '8px', border: '1px solid rgba(0,163,255,0.1)', display: 'flex', gap: '15px', alignItems: 'center' }}>
                <Info size={24} color="var(--projecty-blue)" />
                <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                    <strong>Dica Pro:</strong> Os modelos são gerenciados diretamente pelo processo Ollama controlado pela Project Y. Ao fechar a IDE, o motor é desligado para liberar VRAM.
                </p>
            </div>
        </div>
    );
    ...
    return (
        <div style={modalOverlayStyle}>
            <div style={modalStyle}>
                <div ref={contentRef}>
                    <h2 style={{ marginBottom: '10px' }}>🚀 Bem-vindo ao Project Y</h2>

                    {step === 'install' && (
                        <>
                            <p style={{ marginBottom: '20px', color: 'var(--text-secondary)' }}>
                                Ollama não encontrado no seu sistema. O Project Y precisa dele para rodar modelos locais com privacidade e performance.
                            </p>
...
export default function PlanStepper() {
    ...
    useEffect(() => {
        ...
        const fetchPlan = async () => {
            try {
                const data = await readFile('PROJECTY_PLAN.md');
    ...
    return (
        <div className="plan-stepper" style={{
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span>📝</span>
                    <span>Plano de Execução (PROJECTY_PLAN.md)</span>
                </div>
                <span>{collapsed ? '▼' : '▲'}</span>
            </div>
            ...
            <div className="security-alert-modal" ref={modalRef}>
                ...
                <div className="alert-content">
                    ...
                    <p className="alert-notice">
                        A Project Y detectou uma tentativa de intrusão ou observação não autorizada.
    ...
    return (
        <div className="settings-view">
            <div className="settings-header">
                <img src="./logo.png" alt="Project Y Logo" className="settings-logo projecty-icon" />
                <div>
                    <h2>Configurações</h2>
                    <p>Gerencie as conexões neurais da sua Project Y</p>
                </div>
            </div>
            ...
            <div className="settings-tabs" style={{ display: 'flex', gap: '20px', marginBottom: '25px', borderBottom: '1px solid var(--border)' }}>
                <button
                    ...
                    style={{
                        ...
                        background: 'none',
                        border: 'none',
                        color: activeTab === 'general' ? 'var(--projecty-blue)' : 'var(--text-muted)',
                        borderBottom: activeTab === 'general' ? '2px solid var(--projecty-blue)' : '2px solid transparent',
                ...
                <button
                    ...
                    style={{
                        ...
                        background: 'none',
                        border: 'none',
                        color: activeTab === 'models' ? 'var(--projecty-blue)' : 'var(--text-muted)',
                        borderBottom: activeTab === 'models' ? '2px solid var(--projecty-blue)' : '2px solid transparent',
                    ...
                    <div className="settings-card">
                        ...
                        <div className="settings-card-body">
                            ...
                            <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', marginLeft: '24px' }}>
                                CUIDADO: Isso permite que a IA altere o código-fonte da própria IDE Project Y diretamente.
                            </p>
                        </div>
                    </div>
                            ...
                            <div style={{ display: 'flex', gap: '12px' }}>
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        localStorage.setItem('projecty-theme', 'dark');
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        document.documentElement.setAttribute('data-theme', 'light');
                                        localStorage.setItem('projecty-theme', 'light');
...
export default function StatusBar({ mode, model, workspace, tokenCount, bypassProtection, memoryStatus, onEdgeClick, userName, sentinelStatus = 'active', brainData, motorStatus, onMotorToggle }) {
    ...
    const [motorReadyTrigger, setMotorReadyTrigger] = useState(false);
    const prevMotorStatus = useRef(motorStatus);

    useEffect(() => {
        if (prevMotorStatus.current === 'starting' && motorStatus === 'ready') {
            setMotorReadyTrigger(true);
            setTimeout(() => setMotorReadyTrigger(false), 1000);
        }
    ...
    return (
        <div className="status-bar">
            <div className="status-left">
                ...
                {userName && (
                    <span className="status-item" style={{ color: 'var(--projecty-blue)', display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(0, 163, 255, 0.1)', padding: '0 8px', borderRadius: '12px' }}>
            ...
            <div className="status-right">
                ...
                >
                    <Zap
                        size={12}
                        className={`${motorStatus === 'starting' ? 'pulse-starting' : ''} ${motorReadyTrigger ? 'pulse-ready-trigger' : ''}`}
                        color={motorStatus === 'ready' ? 'var(--projecty-blue)' : motorStatus === 'starting' ? '#ffb088' : 'var(--text-muted)'}
                    />
                ...
                <span className="status-item" title={brainData?.is_ready ? "Memória Persistente Ativa" : "Indexando Código..."} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Brain
                        ...
                        color={brainData?.is_ready ? 'var(--projecty-blue)' : 'var(--accent-orange)'}
                    />
                    BRAIN: <span style={{ color: brainData?.is_ready ? 'var(--projecty-blue)' : 'var(--accent-orange)' }}>{brainData?.is_ready ? 'READY' : 'OFFLINE'}</span>
                </span>
                {/* Library Status (v6.5) */}
                <span className="status-item" title="Biblioteca de Documentação Oficial" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Library size={12} color="var(--projecty-blue)" />
                    LIBRARY: <span style={{ color: 'var(--projecty-blue)' }}>ACTIVE</span>
                </span>
                {/* Mesh Indicator */}
                <span className="status-item" title={isCortex ? "Cortex Node (Alto Processamento)" : "Terminal Node (Processamento Delegado)"} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Network size={12} color="var(--projecty-blue)" />
                    Mesh: {meshNodes} {isCortex ? '(Cortex)' : '(Terminal)'}
                </span>
                {/* Edge QR Shortcut */}
                <span className="status-item clickable" onClick={onEdgeClick} title="Project Y Edge: Acesso Remoto via QR Code" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <span className="status-item brand" style={{ color: 'var(--projecty-blue)', fontWeight: 600 }}>
                    Project Y v7.0
                </span>
            </div>
        </div>
...
export default function SwarmPanel() {
    ...
    return (
        <div className="panel-placeholder" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <div className="panel-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>PROJECT Y MESH (P2P AI)</span>
            ...
            <div style={{ padding: '12px', flex: 1, overflowY: 'auto' }}>
                ...
                {loading && nodes.length === 0 ? (
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '20px' }}>Nenhuma outra Project Y encontrada na rede local.</p>
                ) : (
    ...
    return (
        <div className="telemetry-bar">
            <div className="telemetry-left">
                <img src="./logo.png" alt="Project Y" className="telemetry-logo projecty-icon" />
                <span className="telemetry-brand">Project Y</span>
            </div>
...
```js
/**
 * Project Y Frontend — API Service Layer
 */
...
// ─── Diagnostic Logger ──────────────────────────────────────────────
const _log = (label, ...args) => console.log(`%c[Project Y API] ${label}`, 'color: #7c3aed; font-weight: bold;', ...args);

// ─── Health Check (for diagnostics) ─────────────────────────────────
export const checkHealth = async (retries = 5, delay = 1000) => {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            _log(`⚠️ Health Attempt ${i + 1} failed: ${err.message}`);
...
// ─── Project Y Identity (v1.16/v2.1) ──────────────────────────────────
export const checkIdentityStatus = async () => {
    const response = await fetch(`${BASE}/identity/check`);
...
<head>
  ...
  <meta name="description"
    content="Project Y — Local Intelligence, Global Performance. Ambiente de codificação híbrido com IA local." />
</head>

<body>
  <div id="root"></div>
  <div id="error-overlay" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:red; color:white; padding:20px; z-index:99999; font-family:monospace; white-space:pre-wrap;"></div>
  <script>
    window.addEventListener('error', function(event) {
      document.getElementById('error-overlay').style.display = 'block';
      document.getElementById('error-overlay').innerText = '🔥 REACT CRASH 🔥\n\n' + event.message + '\n\n' + (event.error ? event.error.stack : '');
    });
    window.addEventListener('unhandledrejection', function(event) {
      document.getElementById('error-overlay').style.display = 'block';
      document.getElementById('error-overlay').innerText = '🔥 PROMISE CRASH 🔥\n\n' + event.reason;
    });
  </script>
