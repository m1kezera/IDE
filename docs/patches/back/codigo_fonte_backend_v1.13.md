# Código Fonte Backend Completo - Lumina IDE (v1.13)

> Este documento contém o dump de todo o código-fonte atual exclusivo do Backend em Python da Lumina IDE, refletindo a versão 1.13 com o Lumina Brain (Memória Persistente, LanceDB e busca vetorial).

## Backend - Motor Python FastAPI da Lumina IDE v1.13 (Lumina Brain & LanceDB)
...
class LuminaBrain:
    ...
    async def index_code(self, file_path: str, content: str):
        """Indexa o conteúdo do arquivo no banco de vetores."""
        if not self.is_ready:
            return

        try:
            log.info(f"🧠 Indexando: {file_path}")
            ...
            if TABLE_NAME not in self.db.table_names():
                ...
                # Remove entradas antigas do mesmo arquivo para evitar duplicatas
                # Nota: LanceDB v1+ suporta delete por predicado
                self.table.delete(f'file_path = "{file_path}"')
                self.table.add(data=df)
            
            log.info(f"✅ {len(chunks)} pedaços indexados para {file_path}")
        except Exception as e:
