# Código Fonte Backend Completo - Lumina IDE (v2.0)

> Este documento representa a versão 2.0 (Lumina Shield), consolidando a arquitetura de segurança com Hardware Binding e criptografia de dados críticos.

## Backend - Motor Python FastAPI da Lumina IDE v2.0 (Lumina Shield & Initial Security)
...
class LuminaBrain:
    def __init__(self):
        ...
        self._identity_locked = True # Default to locked until verified
    ...
    async def index_code(self, file_path: str, content: str, source_type: str = "code", tag: str = ""):
        ...
        try:
            log.info(f"🧠 Indexando ({source_type}): {file_path}")
...
def get_hwid() -> str:
    """
    Returns a unique identifier based on the machine's hardware.
    Uses uuid.getnode() which captures the MAC address.
    """
    node = uuid.getnode()
    return str(node)

def generate_master_key(user_name: str, hwid: str) -> str:
    """
    Combines username and hardware ID to generate a 32-byte (AES-256) master key.
    Returns the SHA-256 hash of the combination.
    """
    secret = f"{user_name}:{hwid}"
    return hashlib.sha256(secret.encode()).hexdigest()

def get_identity_key(user_name: str) -> str:
    """Convenience function to get the key for the current hardware."""
    return generate_master_key(user_name, get_hwid())
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Identity Check & Brain Unlock (v1.16)
        ...
        with get_session_direct() as session:
            ...
            if identity:
                if brain.verify_identity(identity.user_name, identity.hwid_hash):
...
def _get_fernet_instance(user_name: str) -> Fernet:
    """Derives a Fernet-compatible key from the master identity key."""
    master_key = get_identity_key(user_name)
    # Fernet requires a 32-byte base64-encoded key.
    # We take the SHA256 digest of the master key and b64 encode it.
    import hashlib
    key_32bytes = hashlib.sha256(master_key.encode()).digest()
    fernet_key = base64.urlsafe_b64encode(key_32bytes)
    return Fernet(fernet_key)

def encrypt_file(file_path: str, user_name: str) -> bool:
    """Encrypts a file in place using the user's identity key."""
    if not os.path.exists(file_path):
        return False
    
    try:
        fernet = _get_fernet_instance(user_name)
...
def decrypt_data(data: bytes, user_name: str) -> bytes:
    ...
    try:
        fernet = _get_fernet_instance(user_name)
        return fernet.decrypt(data[len(b"LUMINA_ENC:"):])
    except Exception as e:
        print(f"Decryption error: {e}")
        # If decryption fails (wrong key/hwid), we return original data 
        # but the logic calling it should handle the failure (e.g. invalid config)
        return data

def protect_project_secrets(project_path: str, user_name: str):
    """Finds .env files and encrypts them."""
    for root, dirs, files in os.walk(project_path):
        for f in files:
            if f == ".env" or f.endswith(".vault"):
                encrypt_file(os.path.join(root, f), user_name)
