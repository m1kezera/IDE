# Project Y v2.4.5 — Backend Source Documentation
> Generated: 2026-03-11 13:27

## Architecture Overview
- **Framework**: FastAPI + Uvicorn
- **Database**: SQLite via SQLModel (stored in %APPDATA%/projecty-ide/pulsyce.db)
- **AI Engine**: Ollama (local) + Cloud API fallback
- **Packaging**: PyInstaller (--onedir)

---

## main.py
**Lines**: 288 | **Size**: 10,322 bytes
...
async def terminal_endpoint(websocket: WebSocket, port: str):
    ...
    try:
        ...
        stdin = process.stdin
        if stdin:
            stdin.write(f"echo [Project Y] Terminal bound to logical port {port}\n".encode('utf-8'))  # type: ignore[arg-type]
            stdin.flush()
...
def start_mesh(port: int = 8000):
    ...
    try:
        ...
        logger.error(f"Failed to start Project Y Mesh: {e}")
